...
void comm_proc(void)
{
   ...
   for (dir = 0; dir < 3; dir++) {
      ...
      for (i = 0; i < num_comm_partners[dir]; i++) {
         if (nonblocking) offset = comm_index[dir][i];
         else offset = 0;
         for (n = 0; n < comm_num[dir][i]; n++)
		 	{send_int[offset+n] = blocks[comm_block[dir][comm_index[dir][i]+n]].new_proc;}
         if (nonblocking) MPI_Isend(&send_int[comm_index[dir][i]], comm_num[dir][i], MPI_INT, comm_partner[dir][i], type, MPI_COMM_WORLD, &s_req[i]);
         if(!nonblocking) MPI_Send(&send_int[0], comm_num[dir][i], MPI_INT, comm_partner[dir][i], type, MPI_COMM_WORLD);
      }
