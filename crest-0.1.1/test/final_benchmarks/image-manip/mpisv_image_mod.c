...
printf("haaaaaa6!\n");
	...
	//CREST_int(sym); //@lavleshm
    for(int i=1;i<size;i++)
      MPI_Ssend(&sym,1,MPI_INT,i,0,MPI_COMM_WORLD);
   }
    else{
   MPI_Recv(&sym,1,MPI_INT,0,0,MPI_COMM_WORLD,&status);
  }
    klee_assume(sym<=10);
	//if(sym > 10) sym = 10;  //@lavleshm
	//if(sym < 2) sym = 2;   //@lavleshm
    klee_assume(sym>=2);
    (*img).iheader.height=sym;
    // Master    
    if(rank == 0)
    {
        // Open the new file
        ...
        Pixel ** newimg = malloc(sizeof(Pixel*) * 10);
...
//    fprintf(stderr, "Rank %d: %d->%d\n",rank,BLOCK_LOW(rank-1,size-1,(*img).iheader.height),BLOCK_HIGH(rank-1,size-1,(*img).iheader.height));
   if(rank != 0)  free(current); //@lavleshm
}
...
int main(int argc, char **argv)
{
    ...
    Pixel ** img = malloc(sizeof(Pixel*) * 10);
