...
void print_phylip_tree(char **tree_description, FILE * tree,
		       sint bootstrap);
void print_nexus_tree(char **tree_description, FILE * tree,
		      sint bootstrap);
sint two_way_split(char **tree_description, FILE * tree, sint start_row,
		   sint flag, sint bootstrap);
sint two_way_split_nexus(char **tree_description, FILE * tree,
			 sint start_row, sint flag, sint bootstrap);
...
static double walltime(double *t0);
...
void phylogenetic_tree(char *phylip_name, char *clustal_name,
		       char *dist_name, char *nexus_name)
/* 
...
*/
{
	...
	if (clustal_name[0] != EOS) {
	    if ((clustal_phy_tree_file =
		 open_explicit_file(clustal_name)) == NULL)
		return;
	} else {
	    if ((clustal_phy_tree_file =
		 open_output_file
		 ("\nEnter name for CLUSTAL    tree output file  ", path,
		  clustal_name, "nj")) == NULL)
		return;
	}
    }
	...
	if (phylip_name[0] != EOS) {
	    if ((phylip_phy_tree_file =
		 open_explicit_file(phylip_name)) == NULL)
		return;
	} else {
	    if ((phylip_phy_tree_file =
		 open_output_file
	...
	if (dist_name[0] != EOS) {
	    if ((distances_phy_tree_file =
		 open_explicit_file(dist_name)) == NULL)
		return;
	} else {
	    if ((distances_phy_tree_file =
		 open_output_file
	...
	if (nexus_name[0] != EOS) {
	    if ((nexus_phy_tree_file =
		 open_explicit_file(nexus_name)) == NULL)
		return;
	} else {
	    if ((nexus_phy_tree_file =
		 open_output_file
    ...
    boot_positions =
	(sint *) ckalloc((seqlen_array[first_seq] + 2) * sizeof(sint));
...
void tree_gap_delete(void)
{				/* flag all positions in alignment that have a gap *//* in ANY sequence */
    sint seqn;
    sint posn;
...
/*********************** First initialisation ***************************/
    ...
    /*
     * Since we are going to pre-compute diq[] and djq[] based on 
     * the current tmat[][], here we will allocate space for 
     * diq[1..(last_seq-first_seq+1)].
     */

    rdiq = (double *)malloc(((last_seq-first_seq+1)+1)*sizeof(double));
    assert(rdiq);


#ifdef SERIAL_NJTREE
        fprintf(stderr,"DEBUG: Using serial codes in nj_tree() ...\n");
	fflush(stderr);
#else
        fprintf(stderr,"DEBUG: Using MPI codes in nj_tree() ...\n");
	fflush(stderr);
#endif
	...
	for (jj = 2; jj <= last_seq - first_seq + 1; ++jj)
		...
		for (ii = 1; ii < jj; ++ii)
			...
			dr = sumd - dij - d2r;
                        ...
                        /* Since fnseqs2 is always (fnseqs - 2), we can
			 * move it out of this loop structure. */
			/* fnseqs2 = fnseqs - 2.0; */
		    ...
		    char *nexus_name)
{
    ...
    if (!output_tree_clustal && !output_tree_phylip && !output_tree_nexus) {
	error
	    ("You must select either clustal or phylip or nexus tree output format");
	return;
    }
    get_path(seqname, path);
...
*/
    ...
    if (usemenu)
	boot_ran_seed =
	    getint("\n\nEnter seed no. for random number generator ", 1,
		   1000, boot_ran_seed);
...
*/
    ...
    if (usemenu)
	boot_ntrials =
	    getint("\n\nEnter number of bootstrap trials ", 1, 10000,
		   boot_ntrials);
	...
	fprintf(clustal_phy_tree_file,
		"the stars in each line show 1 group; ");
...
*/
	...
	compare_tree(standard_tree, sample_tree, boot_totals,
...
/* check if any distances overflowed the distance corrections */
	...
	fprintf(stdout,
		...
		(long) total_overspill, (long) total_dists,
		(long) boot_ntrials);
	fprintf(stdout,
		"\n were out of range for the distance correction.");
...
void compare_tree(char **tree1, char **tree2, sint * hits, sint n)
{
	...
	for (j = 1; j <= n - 3; j++) {
	    ...
	    if ((nhits1 == last_seq - first_seq + 1)
		|| (nhits2 == last_seq - first_seq + 1))
		hits[i]++;
	}
    }
}
...
void print_nexus_tree(char **tree_description, FILE * tree, sint bootstrap)
{
	...
	fprintf(tree, "(%d:%7.5f,%d:%7.5f);", first_seq,
		tmat[first_seq][first_seq + 1], first_seq + 1,
		tmat[first_seq][first_seq + 1]);
    } else {
	...
	old_row =
	    two_way_split_nexus(tree_description, tree,
				last_seq - first_seq + 1 - 2, 1,
				bootstrap);
	fprintf(tree, ":%7.5f", left_branch[last_seq - first_seq + 1 - 2]);
	if ((bootstrap == BS_BRANCH_LABELS) && (old_row > 0)
	    && (boot_totals[old_row] > 0))
	    fprintf(tree, "[%d]", (pint) boot_totals[old_row]);
	fprintf(tree, ",");

	old_row =
	    two_way_split_nexus(tree_description, tree,
				last_seq - first_seq + 1 - 2, 2,
				bootstrap);
...
sint two_way_split_nexus
    (char **tree_description, FILE * tree, sint start_row, sint flag,
     sint bootstrap) {
	...
	old_row =
	    two_way_split_nexus(tree_description, tree, new_row, (sint) 1,
				bootstrap);
	if (start_row == last_seq - first_seq + 1 - 2) {
	    return (new_row);
	}
		       ...
		       sint bootstrap)
{
    ...
    if (last_seq - first_seq + 1 == 2) {
	fprintf(tree, "(%s:%7.5f,%s:%7.5f);", names[first_seq],
		tmat[first_seq][first_seq + 1], names[first_seq + 1],
		tmat[first_seq][first_seq + 1]);
	return;
    }
    ...
    old_row =
	two_way_split(tree_description, tree, last_seq - first_seq + 1 - 2,
		      1, bootstrap);
    fprintf(tree, ":%7.5f", left_branch[last_seq - first_seq + 1 - 2]);
    if ((bootstrap == BS_BRANCH_LABELS) && (old_row > 0)
	&& (boot_totals[old_row] > 0))
...
sint two_way_split
	...
	old_row =
	    two_way_split(tree_description, tree, new_row, (sint) 1,
			  bootstrap);
	if (start_row == last_seq - first_seq + 1 - 2) {
	    return (new_row);
	}
...
void print_tree(char **tree_description, FILE * tree, sint * totals)
{
    ...
    for (col = 1; col <= last_seq - first_seq + 1; col++)
	fprintf(tree, "%1d",
		(pint) tree_description[last_seq - first_seq + 1 -
					2][col]);
    fprintf(tree, "\n");
}
...
*/
{
	...
	standard_tree =
	    (char **) ckalloc((last_seq - first_seq + 2) * sizeof(char *));
	for (i = 0; i < last_seq - first_seq + 2; i++)
	    standard_tree[i] =
		(char *) ckalloc((last_seq - first_seq + 2) *
				 sizeof(char));

        zero = 0.0;
	before = walltime(&zero);

	nj_tree(standard_tree, clustal_phy_tree_file);

	after = walltime(&before);

	fprintf(stderr,"DEBUG: nj_tree() takes %8.6f sec\n",
		after);
