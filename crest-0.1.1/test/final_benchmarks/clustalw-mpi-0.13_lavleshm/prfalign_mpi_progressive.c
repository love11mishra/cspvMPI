...
/*
 * group[] is from the array sets[set] in malign.c,
 * aligned is from the array aligned[] in malign.c,
 * which_set is from the variable "set" in malign.c,
 * dest is the MPI rank of the destination process.
 *
 * Note: return 1 if ok; return 0 if the alignment is skipped
 *       since either nseqs1 or nseqs2 is zero; return -1 on error.
 *      
 *       In order not to change the contents of group[], I pass the
 *       array as group2[] and make a copy of it to group[].
 *
 *
 * Note: "reverse_rank" is the rank of the MPI process which is going to
 *       execute preverse_pass().
 */
lint prf_init(sint *group2, sint * aligned, int which_set, int dest, int reverse_rank)
{
    ...
    group = (sint *)calloc((nseqs+1),sizeof(sint));
    assert(group);
...
*/
    ...
    for (i = 0; i < nseqs; i++)
	if (group[i + 1] == 1)
	    if (seqlen_array[i + 1] > prf_length1)
		prf_length1 = seqlen_array[i + 1];

    nseqs1 = 0;
    if (debug > 0)
	fprintf(stdout, "sequences profile 1:\n");
    for (i = 0; i < nseqs; i++) {
	if (group[i + 1] == 1) {
	    if (debug > 0) {
		extern char **names;
	    ...
	    alignment[nseqs1] =
		(char *) ckalloc((prf_length1 + 2) * sizeof(char));
	    for (j = 0; j < len; j++)
...
*/
    ...
    for (i = 0; i < nseqs; i++)
	if (group[i + 1] == 2)
	    if (seqlen_array[i + 1] > prf_length2)
...
*/
	...
	maxres =
	    get_matrix(matptr, mat_xref, matrix, neg_matrix, int_scale);
	if (maxres == 0)
	    return ((sint) - 1);
/*
...
/* fix suggested by Chanan Rubin at Compugen */
	matrix[mat_xref[0]][mat_xref[4]] =
	    transition_weight * matrix[0][0];
	matrix[mat_xref[4]][mat_xref[0]] =
	    transition_weight * matrix[0][0];
	matrix[mat_xref[2]][mat_xref[11]] =
	    transition_weight * matrix[0][0];
	matrix[mat_xref[11]][mat_xref[2]] =
	    transition_weight * matrix[0][0];
	matrix[mat_xref[2]][mat_xref[12]] =
	    transition_weight * matrix[0][0];
	matrix[mat_xref[12]][mat_xref[2]] =
	    transition_weight * matrix[0][0];
	...
	if (len1 == 0 || len2 == 0) {
		...
		logdiff =
		    1.0 +
		    0.5 * log10((double) ((float) len2 / (float) len1));
	    else if (len1 < len2)
		logdiff =
		    1.0 +
		    0.5 * log10((double) ((float) len1 / (float) len2));
	    else
		logdiff = 1.0;
	    if (logdiff < 0.9)
		logdiff = 0.9;
	...
	if (strcmp(mtrxname, "blosum") == 0) {
	    scale = 0.75;
	    if (negative || distance_tree == FALSE)
		matptr = blosum40mt;
	    else if (pcid > 80.0) {
		matptr = blosum80mt;
	    } else if (pcid > 60.0) {
		...
		if (pcid >= matseries.mat[i].llimit
		    && pcid <= matseries.mat[i].ulimit) {
		    j = i;
		    found = TRUE;
		    break;
		}
	    ...
	    if (debug > 0)
		fprintf(stdout, "pcid %d  matrix %d\n", (pint) pcid,
			(pint) j + 1);
...
/* this gives a scale of 0.5 for pcid=llimit and 1.0 for pcid=ulimit */
	    scale =
		0.5 + (pcid -
		       matseries.mat[j].llimit) /
		((matseries.mat[j].ulimit -
		  matseries.mat[j].llimit) * 2.0);
	} else {
	    matptr = usermat;
	    mat_xref = aa_xref;
	}
	if (debug > 0)
	    fprintf(stdout, "pcid %3.1f scale %3.1f\n", pcid, scale);
	...
	if (negative) {
		...
		gapcoef1 = gapcoef2 =
		    scale * mat_avscore * (float) (gap_open /
						   (logdiff * logmin));
	    lencoef1 = lencoef2 = 100.0 * gap_extend;
	}
    }
	...
	fprintf(stdout,
		"Gap Open1 %d  Gap Open2 %d  Gap Extend1 %d   Gap Extend2 %d\n",
		(pint) gapcoef1, (pint) gapcoef2, (pint) lencoef1,
		(pint) lencoef2);
...
*/
	...
	calc_gap_coeff(alignment, gaps, profile1,
		       (struct_penalties1
			&& use_ss1), gap_penalty_mask1, (sint) 0, nseqs1,
		       prf_length1, gapcoef1, lencoef1);
    else
	calc_gap_coeff(alignment, gaps, profile1,
		       (struct_penalties2
			&& use_ss2), gap_penalty_mask2, (sint) 0, nseqs1,
		       prf_length1, gapcoef1, lencoef1);
/*
...
*/
	...
	for (i = 0; i < prf_length1; i++) {
	    ...
	    fprintf(stdout, "%d %d\n", (pint) profile1[i + 1][GAPCOL],
		    (pint) profile1[i + 1][LENCOL]);
	}
    }
...
*/
	...
	calc_gap_coeff(alignment, gaps, profile2,
		       (struct_penalties2
			&& use_ss2), gap_penalty_mask2, nseqs1,
		       nseqs1 + nseqs2, prf_length2, gapcoef2, lencoef2);
    else
	calc_gap_coeff(alignment, gaps, profile2,
		       (struct_penalties1
			&& use_ss1), gap_penalty_mask1, nseqs1,
		       nseqs1 + nseqs2, prf_length2, gapcoef2, lencoef2);
/*
  calculate the profile matrix.
*/
	...
	for (i = 0; i < prf_length2; i++) {
	    ...
	    fprintf(stdout, "%d %d\n", (pint) profile2[i + 1][GAPCOL],
		    (pint) profile2[i + 1][LENCOL]);
	}
    }
...
/* use Myers and Miller to align two sequences */
    ...
    mybsize = 0;
    mybsize += 17 * sizeof(int);
    mybsize += sizeof(char);
    mybsize += (((LENCOL + 2) * (prf_length1 + 2)) * sizeof(int));
    mybsize += (((LENCOL + 2) * (prf_length2 + 2)) * sizeof(int));
    mybsize += (nseqs + 1) * sizeof(int);

    MPI_Send(&mybsize, 1, MPI_INT, dest, CALLING_STUPID, MPI_COMM_WORLD);

    mpi_buffer = (char *) malloc(mybsize * sizeof(char));
    assert(mpi_buffer);

    position = 0;
    MPI_Pack(&nseqs, 1, MPI_INT, mpi_buffer, mybsize, &position,
	     MPI_COMM_WORLD);
    MPI_Pack(&which_set, 1, MPI_INT, mpi_buffer, mybsize, &position,
	     MPI_COMM_WORLD);
    MPI_Pack(&sb1, 1, MPI_INT, mpi_buffer, mybsize, &position,
	     MPI_COMM_WORLD);
    MPI_Pack(&sb2, 1, MPI_INT, mpi_buffer, mybsize, &position,
	     MPI_COMM_WORLD);
    MPI_Pack(&se1, 1, MPI_INT, mpi_buffer, mybsize, &position,
	     MPI_COMM_WORLD);
    MPI_Pack(&se2, 1, MPI_INT, mpi_buffer, mybsize, &position,
	     MPI_COMM_WORLD);
    MPI_Pack(&profile1[0][GAPCOL], 1, MPI_INT, mpi_buffer, mybsize,
	     &position, MPI_COMM_WORLD);
    MPI_Pack(&profile1[prf_length1][GAPCOL], 1, MPI_INT, mpi_buffer,
	     mybsize, &position, MPI_COMM_WORLD);

    MPI_Pack(&last_print, 1, MPI_INT, mpi_buffer, mybsize, &position,
	     MPI_COMM_WORLD);
    MPI_Pack(&print_ptr, 1, MPI_INT, mpi_buffer, mybsize, &position,
	     MPI_COMM_WORLD);
    MPI_Pack(&max_aln_length, 1, MPI_INT, mpi_buffer, mybsize, &position,
	     MPI_COMM_WORLD);
    MPI_Pack(&max_aa, 1, MPI_INT, mpi_buffer, mybsize, &position,
	     MPI_COMM_WORLD);
    MPI_Pack(&gap_pos1, 1, MPI_INT, mpi_buffer, mybsize, &position,
	     MPI_COMM_WORLD);
    MPI_Pack(&gap_pos2, 1, MPI_INT, mpi_buffer, mybsize, &position,
	     MPI_COMM_WORLD);
    MPI_Pack(&endgappenalties, 1, MPI_CHAR, mpi_buffer, mybsize, &position,
	     MPI_COMM_WORLD);
    MPI_Pack(&prf_length1, 1, MPI_INT, mpi_buffer, mybsize, &position,
	     MPI_COMM_WORLD);
    MPI_Pack(&prf_length2, 1, MPI_INT, mpi_buffer, mybsize, &position,
	     MPI_COMM_WORLD);

    MPI_Pack(&reverse_rank, 1, MPI_INT, mpi_buffer, mybsize, &position,
	     MPI_COMM_WORLD);

    for (i = 0; i < prf_length1 + 2; i++)
	MPI_Pack(profile1[i], (LENCOL + 2), MPI_INT,
		 mpi_buffer, mybsize, &position, MPI_COMM_WORLD);
    for (i = 0; i < prf_length2 + 2; i++)
	MPI_Pack(profile2[i], (LENCOL + 2), MPI_INT,
		 mpi_buffer, mybsize, &position, MPI_COMM_WORLD);

    MPI_Pack(group, (nseqs + 1), MPI_INT,
	     mpi_buffer, mybsize, &position, MPI_COMM_WORLD);

    MPI_Send(mpi_buffer, mybsize, MPI_PACKED, dest, MY_DATA_TAG,
	     MPI_COMM_WORLD);

    free(mpi_buffer);

    /*
     * Seems OK to free() profile1[] and profile2[] here.
     */
...
/*
 * To Receive displ[] from MPI slaves and
 * to update seq_array[][] and seqlen_array[].
 *
 * Note: *from_where is the rank of the sending MPI process,
 *       *which_set is the "set" variable in malign.c.
 */
lint prf_update(int *from_where, int *preverse_rank, int *which_set)//YU:17--22
{
    int mybsize, position;
    char *mpi_buffer;
    MPI_Status status;
    ...
    /* Doing MPI_Recv stuff here ... */

    MPI_Recv(&mybsize, 1, MPI_INT, MPI_ANY_SOURCE, MY_BSIZE_TAG,
	     MPI_COMM_WORLD, &status);
    mpi_buffer = (char *) malloc(mybsize * sizeof(char));
    assert(mpi_buffer);

    *from_where = status.MPI_SOURCE;

    MPI_Recv(mpi_buffer, mybsize, MPI_PACKED, status.MPI_SOURCE,
	     MY_RESULT_TAG, MPI_COMM_WORLD, &status);

    position = 0;
    MPI_Unpack(mpi_buffer, mybsize, &position, &score, 1, MPI_INT,
	       MPI_COMM_WORLD);
    MPI_Unpack(mpi_buffer, mybsize, &position, &print_ptr, 1, MPI_INT,
	       MPI_COMM_WORLD);
    MPI_Unpack(mpi_buffer, mybsize, &position, &last_print, 1, MPI_INT,
	       MPI_COMM_WORLD);
    MPI_Unpack(mpi_buffer, mybsize, &position, &prf_length1, 1, MPI_INT,
	       MPI_COMM_WORLD);
    MPI_Unpack(mpi_buffer, mybsize, &position, &prf_length2, 1, MPI_INT,
	       MPI_COMM_WORLD);
    MPI_Unpack(mpi_buffer, mybsize, &position, &max_aln_length, 1, MPI_INT,
	       MPI_COMM_WORLD);
    MPI_Unpack(mpi_buffer, mybsize, &position, which_set, 1, MPI_INT,
	       MPI_COMM_WORLD);

    MPI_Unpack(mpi_buffer, mybsize, &position, preverse_rank, 1, MPI_INT,
	       MPI_COMM_WORLD);
	...
	fprintf(stderr,"DEBUG: line 712, rank %d, preverse_rank = %d\n", rank,*preverse_rank);
	fflush(stderr);
    }
#endif
    ...
    MPI_Unpack(mpi_buffer, mybsize, &position,
	       displ, (max_aln_length + 1), MPI_INT, MPI_COMM_WORLD);

    group = (sint *) ckalloc((nseqs + 1) * sizeof(sint));
    MPI_Unpack(mpi_buffer, mybsize, &position,
	       group, (nseqs + 1), MPI_INT, MPI_COMM_WORLD);
    ...
    /* 
     * This alignment[][] array was used to be initialized in
     * prf_init(). It has to be re-initialized here for the
     * specific "group".
     */
    alignment = (char **) ckalloc(nseqs * sizeof(char *));
    aln_len = (sint *) ckalloc(nseqs * sizeof(sint));
...
printf("AAAA88888\n");
	...
	if (group[j + 1] == 2) {
	    seqlen_array[j + 1] = prf_length1;
	    seq_array[j + 1] =
		(char *) realloc(seq_array[j + 1],
				 (prf_length1 + 2) * sizeof(char));
	    realloc_seq(j + 1, prf_length1);
	    for (i = 0; i < prf_length1; i++)
		seq_array[j + 1][i + 1] = alignment[NumSeq][i];
	    NumSeq++;
...
/*    gaps = ckfree((void *) gaps); */
	...
	int rank;
	MPI_Comm_rank(MPI_COMM_WORLD,&rank);
	fprintf(stderr,"DEBUG: line 822, rank %d\n", rank);
	fflush(stderr);
    }
#endif
...
*/
	...
	len = alignment_len;
	alignment[j] =
	    (char *) realloc(alignment[j], (len + 2) * sizeof(char));
	for (i = 0; i < len; i++)
	    alignment[j][i] = ta[i];
...
*/
	...
	gap_penalty_mask1 =
	    add_ggaps_mask(gap_penalty_mask1, alignment_len, aln_path1,
			   aln_path2);
    if (struct_penalties1 == SECST)
	sec_struct_mask1 =
	    add_ggaps_mask(sec_struct_mask1, alignment_len, aln_path1,
			   aln_path2);

    if (struct_penalties2 != NONE)
	gap_penalty_mask2 =
	    add_ggaps_mask(gap_penalty_mask2, alignment_len, aln_path2,
			   aln_path1);
    if (struct_penalties2 == SECST)
	sec_struct_mask2 =
	    add_ggaps_mask(sec_struct_mask2, alignment_len, aln_path2,
			   aln_path1);
	...
	for (i = 0; i < nseqs1 + nseqs2; i++) {
		...
		if (alignment[i][j] == ENDALN)
		    break;
		else if ((alignment[i][j] == gap_pos1)
			 || (alignment[i][j] == gap_pos2))
		    c = '-';
		else
		    c = amino_acid_codes[alignment[i][j]];
		fprintf(stdout, "%c", c);
	    }
	    fprintf(stdout, "\n\n");
	}
...
/* if sequence A has just one residue....                                */
	...
	if (go1 == 0)
	    ...
	    midh =
		-gap_penalty2(A + 1, B, 1) - gap_penalty1(A + 1, B + 1, N);
	midj = 0;
	for (j = 1; j <= N; j++) {
...
/* Conquer recursively around midpoint                                   */
	...
	if (debug > 2)
	    fprintf(stdout, "Type 1,1: midj %d\n", (pint) midj);
	pdiff(A, B, midi, midj, go1, 1);
	if (debug > 2)
	    fprintf(stdout, "Type 1,2: midj %d\n", (pint) midj);
	pdiff(A + midi, B + midj, M - midi, N - midj, 1, go2);
    } else {
	if (debug > 2)
	    fprintf(stdout, "Type 2,1: midj %d\n", (pint) midj);
	pdiff(A, B, midi - 1, midj, go1, 0);
	pdel(2);
	if (debug > 2)
	    fprintf(stdout, "Type 2,2: midj %d\n", (pint) midj);
