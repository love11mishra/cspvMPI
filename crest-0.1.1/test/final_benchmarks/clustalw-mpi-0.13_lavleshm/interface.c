...
*/
	...
	if(reset_alignments_new || reset_alignments_all) reset_align();
        ...
        if (quick_pairalign)
           ...
           pairalign((sint)0,nseqs,(sint)0,nseqs);//YU: do pairalign  : generate 0-14 operations, defined in pairalign_new.c
	...
	/***************************************************
	 * When "-usetree=xxxx.dnd" is not used, the following 
	 * malign() will be called.
	 ***************************************************/
	/* count = malign((sint)0,phylip_name); */

	/* 
	 * if "mpirun -np 1" or "mpirun -np 2", we'll call the original
	 * sequential version of malign().
	 *
	 */
	...
	/*
	 * If the environment variable CLUSTALG_PARALLEL_PDIFF is
	 * set to (non-zero), we will call malign_mpi_pdiff(); otherwise
	 * malign_mpi_progressive() will be called.
	 */
	{
	    char *p;
	    char environ[]="CLUSTALG_PARALLEL_PDIFF";
	    p=getenv(environ);
	    printf("HAHA222--%d\n",p );
	    if (p)
	      count = malign_mpi_pdiff((sint)0,phylip_name);
	    else
	      count = malign_mpi_progressive((sint)0,phylip_name);//YU:  the remained communication here!
	}
...
void get_tree(char *phylip_name)
{
	...
	/***************************************************
	 * When we use "-usetree=xxxx.dnd", the following 
	 * malign() will be called.
	 ***************************************************/
	...
	/*
	 * If the environment variable CLUSTALG_PARALLEL_PDIFF is
	 * set (non-zero), we will call malign_mpi_pdiff(); otherwise
	 * malign_mpi_progressive() will be called.
	 */
	{
	    char *p;
	    char environ[]="CLUSTALG_PARALLEL_PROGRESSIVE";
	    p=getenv(environ);
	    if (p)
	      count = malign_mpi_pdiff(0,phylip_name);
	    else
	      count = malign_mpi_progressive(0,phylip_name);
	}
				...
				if((seq_array[fseq][i+fres-1] >=0) && 
				   (seq_array[fseq][i+fres-1] <= max_aa)) {
					...
					for(k=1;res_cat1[k-1]!=NULL;k++) {
							...
							//printf("----------------------%d--------------\n", seq_array[j][i+fres-1]);
							if (seq_array[j][i+fres-1] >= 0 && seq_array[j][i+fres-1] <= 25 && // fix memory out of bound bug by zhenbang
									amino_acid_codes[seq_array[j][i+fres-1]]==c)
							{
								catident1[k-1]++;
								break;
							}
...
/*
 * Terminate all the slave MPI processes
 * */
static int myexit(int rval)
{
   int np,i;
