...
static void reopen(); //added by zhenbang
/*
 *	Global variables
 */
...
/* read the seqno_th. sequence from a PILEUP multiple alignment file */
{
	...
	reopen();// above line is commented to adapt fseek
...
char gline[FILENAMELEN+1];

static void reopen() {
	fclose(fin);
	if((fin=fopen(gline,"r"))==NULL) {
		error("0 Could not open sequence file %s",gline);
	}
}
...
sint readseqs(sint first_seq) /*first_seq is the #no. of the first seq. to read */
{
	...
	if((fin=fopen(line,"r"))==NULL) {
		error("1 Could not open sequence file %s",line);
		return -1;      /* DES -1 => file not found */
	}
	strcpy(seqname,line);
	no_seqs=0;
	check_infile(&no_seqs);
	//added by zhenbang to adapt fseek
	reopen();
	//end
...
/* look for a feature table / gap penalty mask (only if this is a profile) */
	if (profile_no > 0) {
		//rewind(fin); // commented by zhenbang
		fclose(fin);
		if((fin=fopen(line,"r"))==NULL) {
			error("2 Could not open sequence file %s",line);
			return -1;      /* DES -1 => file not found */
		}
