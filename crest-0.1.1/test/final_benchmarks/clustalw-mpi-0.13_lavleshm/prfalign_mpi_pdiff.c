...
lint prfalign_mpi_pdiff(sint * group, sint * aligned)
{
    ...
    /************** DEBUG DEBUG ************************
    fprintf(stderr,"Calling prfalign_mpi_pdiff()...\n");
    fflush(stderr);
    ************** DEBUG DEBUG ************************/
...
/* use Myers and Miller to align two sequences */
    ...
    if (np < 2) {
	    ...
	    /* we'll run pdiff() sequentially. */
         ...
         score = pdiff(sb1, sb2, se1-sb1, 
			 se2-sb2, profile1[0][GAPCOL], profile1[prf_length1][GAPCOL]);
    ...
    /* we'll run pdiff() in parallel. */

    /********** Starting MPI related stuff *****************/
    /*******************************************************/

    /* 
     * We have (np-1) available processes to run pdiff(). Note
     * that process whose rank = 0 does not participate in
     * the following computation.
     */

    avail_p = (int *)calloc(np, sizeof(int));
    assert(avail_p);

    /*
     * avail_p[0] = n: there are n available processes (excluding the one
     *                 whose rank equal to 0)
     * avail_p[1] = 1: the MPI rank of the first available process is 1.
     * avail_p[2] = 2: the MPI rank of the second available process is 2.
     */
    avail_p[0] = (np-1);
    for (i=1;i<np;i++)
	    avail_p[i]=i;
    ...
    arg3 = se1 - sb1;
    arg4 = se2 - sb2;
    arg5 = profile1[0][GAPCOL];
    arg6 = profile1[prf_length1][GAPCOL];
    ...
    /*
     * DEBUG: send to process avail_p[1]
     */
    MPI_Send(&mybsize, 1, MPI_INT, avail_p[1], MALIGN_TAG, MPI_COMM_WORLD);
    mpi_buffer = (char *)malloc(mybsize*sizeof(char));
    assert(mpi_buffer);

    position = 0;

    MPI_Pack(&prf_length1, 1, MPI_INT, mpi_buffer, mybsize, &position, MPI_COMM_WORLD);
    MPI_Pack(&prf_length2, 1, MPI_INT, mpi_buffer, mybsize, &position, MPI_COMM_WORLD);
    ...
    MPI_Pack(&arg1, 1, MPI_INT, mpi_buffer, mybsize, &position, MPI_COMM_WORLD);
    MPI_Pack(&arg2, 1, MPI_INT, mpi_buffer, mybsize, &position, MPI_COMM_WORLD);
    MPI_Pack(&arg3, 1, MPI_INT, mpi_buffer, mybsize, &position, MPI_COMM_WORLD);
    MPI_Pack(&arg4, 1, MPI_INT, mpi_buffer, mybsize, &position, MPI_COMM_WORLD);
    MPI_Pack(&arg5, 1, MPI_INT, mpi_buffer, mybsize, &position, MPI_COMM_WORLD);
    MPI_Pack(&arg6, 1, MPI_INT, mpi_buffer, mybsize, &position, MPI_COMM_WORLD);
    MPI_Pack(&endgappenalties, 1, MPI_CHAR, mpi_buffer, mybsize, 
		    &position, MPI_COMM_WORLD);
    MPI_Pack(&max_aa, 1, MPI_INT, mpi_buffer, mybsize, &position, MPI_COMM_WORLD);


    for (i = 0; i < prf_length1 + 2; i++)
	MPI_Pack(profile1[i], (LENCOL + 2), MPI_INT, mpi_buffer,
			mybsize, &position, MPI_COMM_WORLD);
    for (i = 0; i < prf_length2 + 2; i++)
	MPI_Pack(profile2[i], (LENCOL + 2), MPI_INT, mpi_buffer,
			mybsize, &position, MPI_COMM_WORLD);

    MPI_Pack(&avail_p[0], 1, MPI_INT, mpi_buffer, mybsize, &position, MPI_COMM_WORLD);
    MPI_Pack(avail_p, np, MPI_INT, mpi_buffer, mybsize, &position, MPI_COMM_WORLD);


    MPI_Send(mpi_buffer, mybsize, MPI_PACKED, avail_p[1], MY_DATA_TAG,
				 MPI_COMM_WORLD);
    free(mpi_buffer);


    /**************************************************************
    score = pdiff(sb1, sb2, se1 - sb1, se2 - sb2, profile1[0][GAPCOL],
	      profile1[prf_length1][GAPCOL]);
    ***************************************************************/
    ...
    MPI_Recv(&mybsize, 1, MPI_INT, avail_p[1], MY_RESULT_TAG, 
    ...
    position=0;
    MPI_Recv(mpi_buffer, mybsize, MPI_PACKED, avail_p[1], MY_RESULT_TAG,
                     MPI_COMM_WORLD, &status);
    ...
    */
    ...
    MPI_Unpack(mpi_buffer, mybsize, &position,
                      pa, 3*max_aln_length, MPI_INT, MPI_COMM_WORLD);
    ...
    /*
    MPI_Unpack(mpi_buffer, mybsize, &position,
                      &print_ptr, 1, MPI_INT, MPI_COMM_WORLD);
    MPI_Unpack(mpi_buffer, mybsize, &position,
                      &last_print, 1, MPI_INT, MPI_COMM_WORLD);
    */

    free(mpi_buffer);

    /*
     * Reconstruct displ[] from pa[]: print_ptr and last_print
     * will be modified here.
     */

    /* Do palign(), padd() and pdel() according to
     * the information in pa[] */
