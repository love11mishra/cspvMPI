 ...
 *
 * This version erroneously sets upper and lower neighbors; on boundaries,
 * these should be MPI_PROC_NULL.
 *
...
#define nxlg   10    // number of x coordinates (including boundary)
...
void initdata() {
  ...
  for (row = 0; row < nylg; row++)
    for (col = 0; col < nxlg; col++)
      old[row][col] = new[row][col] = 0;
  if (rank == 0)
    for (col = 0; col < nxlg; col++)
      old[0][col] = new[0][col] = 10;
  if (rank == nprocs-1)
    for (col = 0; col < nxlg; col++)
      old[nylg-1][col] = new[nylg-1][col] = 10;
  if (rank == 0) {
    printf("nprocs  = %d\n", nprocs);
    printf("nxlg    = %d\n", nxlg);
    printf("nylg    = %d\n", nylg);
