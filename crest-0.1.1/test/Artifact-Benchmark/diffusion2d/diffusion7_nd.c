...
//#include "mpi.h"
#define SQUARE(x) ((x)*(x))
#define nprocsx 1   /* number of processes, x direction           */
#define nprocsy 7   /* number of processes, y direction           */
...
void write_frame_2(int time,int rank, double u[nxlg][nylg]) {
  int i, j, k, m, n, from;
  double buf[nxl];
...
void write_frame_1(int time,int rank, double u[nxlg][nylg]) {
  int i, j, k, m, n, from;
  double buf[nxl];
...
void exchange_ghost_cells(double u[nxlg][nylg], int up, int down,int left, int right) {
  int i, j;
...
void update(double u[nxlg][nylg]) {
  int i, j;
  double k = D*dt/(dx*dx);
