...
void write_frame_2(int time,int rank, double u[/*nxlg*/4][/*nylg*/4]) {
  int i, j, k, m, n, from;
  double buf[/*nxl*/2];
  ...
  if (rank != 0) {
   ...
   // file = fopen(filename, "w");
    ...
    for (n = 0; n < nprocsy; n++)
      for (j = 1; j <= nyl; j++) {
        for (m = 0; m < nprocsx; m++) {
          from = n*nprocsx + m;
          if (from != 0)
            for (i = 1; i <= nxl; i++) fprintf(stdout, "        ");
          else
            for (i = 1; i <= nxl; i++) fprintf(stdout, "%8.2f", u[i][j]);
        }
        fprintf(stdout, "\n");
      }
    ...
    //  fseek(file, row*linelen + procx*nxl*8, SEEK_SET);
      for (i = 1; i <= nxl; i++) fprintf(stdout, "%8.2f", buf[i-1]);
    }
    //fclose(file);
  }
}

void write_frame_1(int time,int rank, double u[/*nxlg*/4][/*nylg*/4]) {
  int i, j, k, m, n, from;
  double buf[/*nxl*/2];
...
void exchange_ghost_cells(double u[/*nxlg*/4][/*nylg*/4], int up, int down,int left, int right) {
  int i, j;
  double sbufx[/*nxl*/2], rbufx[/*nxl*/2], sbufy[/*nyl*/2], rbufy[/*nyl*/2];
...
void update(double u[/*nxlg*/4][/*nylg*/4]) {
  int i, j;
  double k = D*dt/(dx*dx);
  double u_new[/*nxlg*/4][/*nylg*/4];
...
int main(int argc,char *argv[]) {
  ...
  write_frame(0,rank,u);
  MPI_Barrier(MPI_COMM_WORLD);
  for (iter = 1; iter <= nsteps; iter++) {
    exchange_ghost_cells(u,up,down,left,right);
    //MPI_Barrier(MPI_COMM_WORLD);
    update(u);
    //MPI_Barrier(MPI_COMM_WORLD); 
    // write_frame(iter);
