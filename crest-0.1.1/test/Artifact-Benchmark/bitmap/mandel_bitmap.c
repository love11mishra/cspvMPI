...
Compression: %u\n\t\
Number of colors: %u\n\t\
Important colors: %u\n", linebytes, (*bmp).header.type, (*bmp).header.size, (*bmp).header.offset, (*bmp).iheader.size,
           (*bmp).header.reserved1, (*bmp).header.reserved2, (*bmp).iheader.width, (*bmp).iheader.height,
           (*bmp).iheader.isize,
...
// memset(colors,0,linebytes);
        ...
        do {
            ...
            fprintf(stderr, "[%d] Recv'd row: %d from %d. Count = %d, row = %d %f s/row %f left\n",
                    rank, status.MPI_TAG, status.MPI_SOURCE, count, row, (end - start) / (double) row,
                    (end - start) / (double) row * (bmp.iheader.height - row));
            count--;
