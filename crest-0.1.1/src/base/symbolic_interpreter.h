...
class SymbolicInterpreter {
 public:
  ...
  void DumpPCFromBranch(SymbolicPred*); //pkalita
  void addArrayElemMap(addr_t, int, type_t); //pkalita
  addr_t isArrayElemPresent(addr_t); //pkalita
  bool isAlreadySymbolic(addr_t); //@lalveshm
  bool setAddrToSval(addr_t, int); //@lavleshm
  int getAddrToSval(addr_t);  //@lavleshm
  int getDataTypeSize(type_t ); //pkalita
 ...
 private:
  ...
  //To map array element to its starting address @pkalita
  map<addr_t, std::pair<int, type_t>> arrayElemMap_;
  
  // @lavleshm Map<adress, symbolic_variable_number> [symbolic_variable_number = value of num_input_ at the time of making symbolic]
  map<addr_t, int> addrToSval;
